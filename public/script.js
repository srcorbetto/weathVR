  ...
  trigger[i].addEventListener("click", e => {
    ...
    $.get(`/weather?id=${query}`, data => {
      ...
      if (
        ...
        weather.sunsetComparison < weather.sunriseComparison
      ) {
        weather.night = true;
      ...
      if (weather.weatherIconKey === "8" && weather.night === true) {
        dataIcon.setAttribute(
          "src",
          `/assets/icons/${weather.weatherIconKey}n.png`
        );
      } else {
        dataIcon.setAttribute(
          "src",
          `/assets/icons/${weather.weatherIconKey}.png`
        );
      }
      ...
      for (let j = 0; j < object.length; j++) {
        object[j].setAttribute(
          "gltf-model",
          weather.models[weather.weatherIconKey].object
        );
        object[j].setAttribute(
          "scale",
          weather.models[weather.weatherIconKey].scale
        );
      }

      // set precipitation
      if (
        weather.weatherIconKey === "2" ||
        weather.weatherIconKey === "3" ||
        weather.weatherIconKey === "5" ||
        weather.weatherIconKey === "6"
      ) {
